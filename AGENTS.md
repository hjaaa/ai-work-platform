...
- `.claude/settings.json` 注册了 PreToolUse hook（`.claude/hooks/check-protected-branch.sh`）：在 `develop` / `main` / `master` 分支上，对 Edit / Write / NotebookEdit 一律 deny。这是刻意的护栏，遇到拒绝应切 feature 分支再改，**不要改用 shell 命令绕过**。
