...
- **Java**（各 `ai-work-*` 后端模块）→ `context/team/dev-standards/java/`（源自《Java 开发手册（黄山版）》），含编程规约：命名、常量、格式、OOP、日期时间、集合、并发、控制语句、注释、前后端交互、其他（`01`~`11`）；错误码、异常与日志（`12`~`14`）；单元测试（`15`）；安全（`16`）；工程结构（`17`~`19`）；设计规约（`20`）；专有名词与错误码列表附录（`21`~`22`）
