...
CREATE TABLE `baas_project` (
  ...
  `ddl_fence_epoch` bigint NOT NULL DEFAULT 0 COMMENT '项目级单调 fencing 计数(spec §9.2)',
...
CREATE TABLE `baas_ddl_log` (
  ...
  `table_id` bigint DEFAULT NULL COMMENT '不可变目标表 ID(baas_table.id)',
  ...
  `fence_epoch` bigint DEFAULT NULL COMMENT '取得所有权时的项目 epoch;历史/PENDING 为 NULL',
