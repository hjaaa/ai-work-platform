...
import com.aiwork.admin.api.entity.SysApiKey;
import com.aiwork.admin.api.feign.RemoteApiKeyService;
...
import com.aiwork.common.core.util.SpringContextHolder;
import com.aiwork.common.core.util.WebUtils;
import com.aiwork.common.security.service.AiWorkDefaultUserDetailsServiceImpl;
import com.aiwork.common.security.service.AiWorkUser;
import com.aiwork.common.security.service.AiWorkUserDetailsService;
...
public class AiWorkCustomOpaqueTokenIntrospector implements OpaqueTokenIntrospector {
	...
	public OAuth2AuthenticatedPrincipal introspect(String token) {
		...
		if (AuthorizationGrantType.CLIENT_CREDENTIALS.equals(oldAuthorization.getAuthorizationGrantType())) {
			return new AiWorkClientCredentialsOAuth2AuthenticatedPrincipal(
		...
		Map<String, AiWorkUserDetailsService> userDetailsServiceMap = SpringContextHolder
			.getBeansOfType(AiWorkUserDetailsService.class);

		Optional<AiWorkUserDetailsService> optional = userDetailsServiceMap.values()
			.stream()
		...
		try {
			...
			userDetails = optional.get().loadUserByUser((AiWorkUser) tokenPrincipal);
		}
		...
		// 注入客户端信息，方便上下文中获取
		AiWorkUser aiworkUser = (AiWorkUser) userDetails;

		Objects.requireNonNull(aiworkUser)
			.getAttributes()
	...
	private OAuth2AuthenticatedPrincipal handleApiKeyToken(String token) {
		...
		UserDetails userDetails = SpringContextHolder.getBean(AiWorkDefaultUserDetailsServiceImpl.class)
			.loadUserByUsername(apiKey.getUsername());
		if (!(userDetails instanceof AiWorkUser aiworkUser)) {
			throw new InvalidBearerTokenException("API Key owner not found");
		}

		if (!aiworkUser.isAccountNonLocked()) {
			log.warn("用户账号 {} 已被锁定，拒绝 API Key 访问", aiworkUser.getUsername());
			throw new InvalidBearerTokenException("Account is locked");
		}
		if (!aiworkUser.isAccountNonExpired()) {
			log.warn("用户账号 {} 已过期，拒绝 API Key 访问", aiworkUser.getUsername());
		...
		aiworkUser.getAttributes().put(SecurityConstants.CLIENT_ID, "apikey");
		return aiworkUser;
	}
