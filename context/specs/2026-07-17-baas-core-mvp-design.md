...
- 日期:2026-07-17
- 状态:v10,已按四轮评审意见修订,并附实施规划与会话交接(§16);v8 补齐 Plan B(表管理与 DDL)设计细化,v9/v10 按 Plan B 两轮设计评审修订
...
| `baas_ddl_log` | Schema 操作日志:project_id + 操作 ID 联合唯一(幂等键)、操作类型(create/alter/drop/acl-index/cleanup-drop)、目标表名、**request_hash**(操作指纹,见 9.2)、**result_snapshot**(成功结果 JSON,幂等重放返回)、**owner_token**(执行者标识,陈旧 RUNNING 接管用,见 9.2)、DDL 内容(**默认值字面量以占位符脱敏记录**)、检查点步骤(PREPARED/DDL_APPLIED/METADATA_APPLIED,见 9.2)、状态、错误信息、重试次数。**Plan B 需附 ALTER 迁移脚本**:本表补新增字段;同时扩容 Plan A 建的 `baas_table.comment` → varchar(2048)、`baas_column.comment` → varchar(1024)、`baas_column.default_value` → text(现均为 varchar(255),小于 §7.3 API 允许上限,会导致项目库 DDL 成功后平台元数据写入失败) |
...
- 设置 `ownerColumn` 时校验:列存在、类型 bigint、`anon.insert=true` 时列必须可空;**若该列尚无索引(含唯一索引)则自动补建**——补索引是 DDL,走 §9.2 的 DDL 锁与 `baas_ddl_log`,幂等键即本次请求的客户端 `operationId`(操作类型 acl-index)。**执行前必须探测项目库索引现状**:已存在则跳过 DDL 只写元数据——是否补索引由现状决定,不以历史 SUCCESS 记录判断(否则「owner 取消 → 索引被删 → 再次设置 owner」场景无法重建索引)
...
- **串行锁(双层)**:第一层 Redis 分布式锁,锁 key `baas:ddl:{projectId}`,value = 操作 ID(防误释放);TTL 60 秒 + watchdog 自动续租(每 20 秒),**续租必须为原子 compare-and-expire**(Lua:value 匹配才延期,禁止 GET+EXPIRE 两步);获取失败返回 409「该项目有 DDL 操作进行中」。**每个检查点推进前校验锁仍为本操作持有**,续租失败或校验不通过立即中止后续步骤。但 Redis 层无法中止**正在执行中**的 DDL(JDBC 阻塞在语句上,取消操作亦不可靠),因此第二层在**执行 DDL 的同一项目库连接**上先取 `GET_LOCK('baas_ddl_{projectId}', 0)`,获取失败立即中止;DDL 完成后 `RELEASE_LOCK`,连接断开自动释放——即使 Redis 租约过期、后继操作抢到 Redis 锁,它在数据库边界拿不到 advisory lock,**不会与旧 DDL 并发**
...
- **陈旧 RUNNING 接管**:进程崩溃会留下无人推进的 RUNNING 记录(锁过期后表将永久停在 CREATING/ALTERING)。`baas_ddl_log.owner_token` 记录执行者(实例标识 + 随机值);同操作 ID + 同指纹的重试请求遇到 RUNNING 时,若确认 Redis 锁已失效(key 不存在或非该操作持有)且能取得双层锁,则通过**条件 UPDATE(CAS owner_token)**接管该记录并按检查点探测续跑;接管失败或锁仍被持有 → 409
