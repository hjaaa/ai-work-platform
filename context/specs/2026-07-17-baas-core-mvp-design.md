...
- 日期:2026-07-17
- 状态:v45;v1~v5 经四轮整体评审定稿,v6 附实施规划与会话交接(§16),v8 补齐 Plan B(表管理与 DDL)设计细化,v9~v25 按 Plan B 十七轮设计评审修订,v26 为四维系统性自查修订,v27 为 Plan B 实施计划复审闭环修订,v28 补齐 Plan C(数据面 REST)设计细化,v29 按 Plan C 设计评审(2 P0/5 P1)修订,v30 按复审(1 P0/1 P1)修订,v31 按三轮复审(1 P1)修订,v32 补齐 Plan D(终端用户 Auth)设计细化(含终端用户软删/恢复与系统表 manifest v3),v33 按 Plan D 设计评审(1 P0/6 P1)修订,v34 按二轮评审(3 P1)修订,v35 按三轮评审(1 P1,发布协议补平台库迁移)修订,v36 按 Plan D 四轮评审(9 P1 甄别收敛)修订(修订记录补记),v37 补齐 Plan E(Studio 前端)设计细化,v38 按 Plan E 设计评审(codex,3 P2 甄别确认)修订,v39 按 Plan E 二轮评审(codex,3 P2 甄别确认)修订,v40 按 Plan E 三轮评审(codex,1 P2 确认修订、1 P1 甄别为误报)修订,v41 按 Plan E 四轮评审(codex,2 P1 确认修订)修订,v42 按 Plan E 五轮评审(codex,2 P1 + 1 P2 确认修订)修订,v43 按 Plan E 六轮评审(codex,2 P2 确认修订)修订,v44 按 Plan E 七轮评审(codex,2 P2 确认修订)修订,v45 按 Plan E 八轮评审(codex,1 P2 确认修订,连续 P2 终止)修订
...
- 项目列表页:members 页三件套范式(搜索卡 + 表格卡 + 创建弹窗);状态列覆盖**列表 API 实际可返回的项目状态**(`PROVISIONING/ACTIVE/MIGRATING/FAILED/DELETING`——`ProjectAccessService.listVisible` 与单查均只 `ne DELETED`,故 DELETED 外全部可返回;其中 `MIGRATING`(系统表迁移中、数据面阻断)尤须有明确标签,不能留空/未知),浅底深字标签;`PROVISIONING → ACTIVE` 由用户手动刷新,MVP 不做轮询
...
**接入层**:新增 `src/api/baas/{project,table,apiKey,endUser}.ts`,沿用 `utils/request.ts` 统一拦截(`R<T>` 解包、token 注入、401 跳转);**同步长操作(建/改/删表、ACL 配置、手动对账、建项目等走 §9.2 DDL 通道或同步执行的调用)不沿用 `utils/request.ts` 默认 30s Axios 超时**——后端此类操作当前**无端到端时限**:实测 `ProjectProvisioner` 顺序执行建库/建账号/授权/初始化系统表多条 DDL、其 `JdbcTemplate` 未设任何 queryTimeout,对账亦可执行不定长 DB 序列,`baas.ddl.execute-timeout-seconds`(§13)仅是表 DDL 引擎的 per-statement 上限、约束不到 provisioner/reconcile,故不存在可供客户端「覆盖最坏耗时」的有限界。Plan E 范围内(纯前端)对这些调用**取消 client 端 deadline(`timeout: 0`,由用户主动中止或连接断链兜底)**,确保建项目响应(含两枚一次性 key)与慢 ALTER/对账结果不被客户端提前 abort 误报失败;**更稳妥的方案——后端为这些请求加有界总时限 + 建项目响应丢失后的恢复/幂等(可再取初始 key)——属后端契约改动,依本节「后端契约问题单独立项修复,不混入 Plan E」记为 follow-up**;新增 env `VITE_BAAS_PATH` 默认 `/baas`(cloud 形态走网关剥首段),boot 单体部署覆盖为 `/admin`(同 `VITE_AUTH_PATH` 先例)。
...
- 表级 ACL 配置:anon/authenticated × select/insert/update/delete 开关矩阵 + `owner_column` 下拉(仅 bigint 列可选,**且须排除主键 `id`**——`AclConfigService.putAcl` 拒 `ownerColumn == "id"` 返回 400(§8.3),下拉不得把 id 作为可选项);后端约束(anon.insert 要求可空、owner 自动建索引等)违反时直显后端 msg

**危险操作确认分级**:紧急轮换(**文案须精确到「旧 access JWT 立即失效」,不得说成「全部 token 失效」**——§6.1 明确 `_sessions` 与 refresh token 不撤销,合法客户端凭 refresh token 立即换取新 key 签发的 access JWT)、删除项目须**输入项目 ref** 方可提交;删表须**输入表名**(文案说明软删 tombstone、同名禁重建);常规轮换、Key 吊销、用户软删/恢复用 confirm,文案说明后果(软删撤销全部会话且邮箱不释放,**但须显式告知该用户已签发的 access JWT 在剩余 TTL(≤1 小时)内仍可访问 `/rest`——数据面不回查 `_users`(§7.3),要立即切断访问须叠加紧急轮换**;restore 后旧会话不复活)。
...
**横切契约**:operationId 为前端生成的 UUID v4——**secure context(HTTPS/localhost)用 `crypto.randomUUID()`;§5 明文 HTTP 部署(`http://gw:9999`、`http://host:9999`,非 localhost 即非 secure context,`crypto.randomUUID` 在此为 undefined)必须回退到基于 `crypto.getRandomValues()` 的 UUID v4 生成(该 API 不受 secure context 限制),否则 operationId 生成抛错、建/改/删表与 ACL/对账全部无法提交**;同一次用户意图的重试**复用同一 ID**(幂等重放拿原快照);**409 须按后端 msg 区分,不可统一「刷新重试」**——仅 DDL 锁忙(§9.2「该项目有 DDL 操作进行中」)提示「操作冲突或锁忙,请刷新后重试」;指纹不一致(§7.3)、有损 ALTER 数据不兼容/可空→非空含 NULL(§13)、数据面唯一键冲突(§11)等**非锁 409 一律直显后端 msg**(这些场景「刷新重试」无效、会把用户带入重试死循环);其余错误经统一拦截直显后端 msg。
...
- **Plan E(Studio 前端)必测**(Vitest 单测,前端门禁):**改表操作列表组装**(重命名与删+加的区分、同列多操作互斥在前端拦截、无操作禁提交、组装出的 PATCH body 与 §7.3 契约字段逐项一致);**列定义校验边界**(varchar 长度 1/4096/越界、decimal 精度/标度约束、text/json 默认值拦截、datetime `CURRENT_TIMESTAMP` 特例);**allowLossy 重发流程**(识别「需要 allowLossy」的 400 → 确认后同 operationId 重发,body 仅 `allowLossy` 变化);**`VITE_BAAS_PATH` 前缀拼接**(默认 `/baas` 与 boot 覆盖 `/admin`);detail 静态子路由注册与守卫回归(并入现有 guard/menuPaths spec 覆盖);**侧边栏前缀高亮**(`/baas/projects/:ref` 详情路由下父菜单 `/baas/projects` 高亮、`/baas/projects-x` 类同前缀平级路径不误亮);**`allowed_origins` 通配/白名单往返**(`ProjectVO` 回 `null` → 通配开关开启、回 `[]` → 开关关闭且空列表、编辑提交值与开关态一致,不把通配存成 deny-all);**建表 body 剔除 id 行**(只读 id 行不进 `columns`,POST body 断言无 `id` 列定义);**409 分类提示**(锁忙 409 → 「刷新重试」文案、指纹/有损/唯一键等非锁 409 → 直显后端 msg,不误导重试);**创建响应双 key 展示**(建项目弹窗同时展示 publishable 与 secret 明文并可复制);**项目状态标签**(列表 API 可见的 `PROVISIONING/ACTIVE/MIGRATING/FAILED/DELETING` 各有标签样式——DELETED 被 `listVisible` 过滤不可达、不纳入,MIGRATING 有明确标签不留空,PROVISIONING 文案与手动刷新提示正确);**operationId 明文 HTTP 回退**(`crypto.randomUUID` 缺失时以 `crypto.getRandomValues` 生成合法 UUID v4,重试复用同一 ID);**长操作 timeout 覆盖**(DDL/对账/建项目类调用取消 client deadline(`timeout: 0`)、不落 `utils/request.ts` 30s 默认——断言慢响应下建项目响应不被客户端提前 abort);**owner 列下拉排除 id**(仅 bigint 且不含主键 id,避免选中必被后端 400 拒的 id)。`npm run lint` / `npm run build`(vue-tsc)/ `npm run test:unit -- run` 三门禁全绿;依赖变更须干净环境 `npm ci` 验证
...
- **v45(2026-07-23)**:按 Plan E(Studio 前端)八轮设计评审(codex,1 P2 确认;本轮达成连续 3 轮仅 P2、终止对抗循环)修订。P2——长操作 client 超时无法用有限值「覆盖端到端最坏耗时」:后端 provisioning/reconcile 本就无端到端时限(实测 `ProjectProvisioner` 多条 DDL 的 `JdbcTemplate` 无任何 queryTimeout、reconcile 不定长),MySQL 长 stall 时任何有限 client 超时仍会先 abort、丢失建项目一次性 key → §7.7 接入层与 §14 改为 Plan E 范围内对这些调用取消 client deadline(`timeout: 0`,由用户中止/断链兜底),并把「后端加有界总时限 + 建项目响应丢失恢复/幂等」按 §7.7「后端契约问题单独立项」记为 follow-up、不混入 Plan E。此为纯前端设计细节修正,不改后端契约与已合入代码。
- **v44(2026-07-23)**:按 Plan E(Studio 前端)七轮设计评审(codex,2 P2 确认,逐条对照 develop 已合入代码核实)修订。两条均为 v42/v43 修复的遗漏与边界收紧。P2——① 项目状态列漏 `MIGRATING`:实测 `ProjectStatus` 枚举含 `MIGRATING`(系统表迁移中、数据面阻断),`listVisible` 只 `ne DELETED` 故 MIGRATING 会返回,v42 仅列四态会在迁移期显示空白/未知 → §7.7 line 293 与 §14 补 `MIGRATING` 标签(五态);② 长操作 client 超时由 `baas.ddl.execute-timeout-seconds + 余量` 推导不成立:实测 `ProjectProvisioner`(建库/建账号/授权/初始化系统表多条 DDL)的 `JdbcTemplate` 未套用该 per-statement 超时,该值既非整请求上限也不覆盖 provisioner,建项目仍会在返回一次性 key 前 abort → §7.7 接入层与 §14 改为「独立可配、覆盖端到端最坏耗时的 client 超时,不由后端单条 DDL 超时推导」。本轮均为纯前端设计细节修正,不改后端契约与已合入代码。
- **v43(2026-07-23)**:按 Plan E(Studio 前端)六轮设计评审(codex,2 P2 确认,逐条对照 develop 已合入代码核实)修订。两条均为 v42 修复的边界收紧。P2——① 长操作 client timeout 定为「≥300s」不够:后端 DDL 超时可配(实测 `DdlExecutionEngine` 与 `ProjectLifecycleService` 均 `@Value("${baas.ddl.execute-timeout-seconds:300}")`)且命中超时后还需转译失败、更新 DDL 日志与表状态、提交、返回,client 恰好等于该值会先 abort、重现误报失败/丢结果 → §7.7 接入层与 §14 改为 client timeout 严格大于后端可配超时并留明确余量(后端超时 + 30~60s 缓冲);② owner 列下拉仅按 bigint 过滤会把主键 `id` 列出,而 `AclConfigService.putAcl` 拒 `ownerColumn == "id"` 返回 400(实测,§8.3),该选项必失败 → §7.7 表级 ACL 配置与 §14 明确 owner 下拉排除主键 id。本轮均为纯前端设计细节修正,不改后端契约与已合入代码。
- **v42(2026-07-23)**:按 Plan E(Studio 前端)五轮设计评审(codex,2 P1 + 1 P2 确认,逐条对照 develop 已合入代码核实)修订。P1——① operationId 依赖 `crypto.randomUUID()`,但 §5 部署 URL 为明文 HTTP(`http://gw:9999`、`http://host:9999`,非 localhost 即非 secure context,该 API 为 undefined),会使建/改/删表与 ACL/对账全部无法提交:§7.7 横切契约改为 secure context 用 `crypto.randomUUID`、明文 HTTP 回退 `crypto.getRandomValues` 生成 UUID v4(后者不受 secure context 限制),§14 增回退断言;② 接入层沿用 `utils/request.ts` 默认 30s Axios 超时(实测 `timeout: 30000`),但同步 ALTER/对账可执行至后端 5 分钟 DDL 超时(§13),>30s 会 UI 误报失败而服务端仍提交、慢建项目更丢弃含两枚一次性 key 的响应:§7.7 接入层要求 DDL/对账/建项目类调用设 per-call ≥300s timeout,§14 增 timeout 覆盖断言。P2——③ v40 项目状态列枚举含 `DELETED`,但 `ProjectAccessService.listVisible` 与单查均 `ne DELETED`(实测),DELETED 项目对 Studio 不可见、该标签与测试不可达:§7.7 line 293 与 §14 状态标签回退为列表 API 实际可返回的 `PROVISIONING/ACTIVE/FAILED/DELETING` 四态。本轮均为纯前端设计细节修正,不改后端契约与已合入代码。
- **v41(2026-07-23)**:按 Plan E(Studio 前端)四轮设计评审(codex,2 P1 确认,逐条对照 spec §6.1/§7.3/§14 核实)修订。两条均为 §7.7 危险操作确认弹窗**文案与安全语义相悖**、在安全事件中会误导运维,判定确认。P1——① 紧急轮换文案「全部终端用户 token 立即失效」失准:§6.1 明确紧急轮换仅撤销 JWT 签名密钥使**旧 access JWT** 立即失效,`_sessions` 与 refresh token 不撤销,合法客户端凭 refresh token 立即换取新 key 签发的 access JWT(§14 亦有「紧急轮换后 refresh token 仍可换新」必测项)→ line 306 文案精确为「旧 access JWT 立即失效」并注明会话/refresh token 存活;② 软删文案仅说「撤销全部会话」,漏披露 §7.3 既定语义——软删用户已签发的 access JWT 在剩余 TTL(≤1 小时)内仍可访问数据面 `/rest`(数据面不回查 `_users`),运维误以为软删即时断访 → line 306 文案增补 access JWT 存活窗口告知,并指明立即切断须叠加紧急轮换。本轮仅 P1 两处纯前端确认文案精确化,不改后端契约与已合入代码。
- **v40(2026-07-23)**:按 Plan E(Studio 前端)三轮设计评审(codex,1 P2 确认 + 1 P1 误报,逐条对照 develop 已合入代码与 git 历史核实)修订。P2(确认)——项目列表状态列误用表状态名 `CREATING`(实测项目状态机 `ProjectStatus` 为 `PROVISIONING/ACTIVE/FAILED/DELETING/DELETED`、`ProjectLifecycleService` 走 `PROVISIONING → ACTIVE`,`CREATING` 属 §9.5 表状态机;spec §9.1 line 349 亦为 PROVISIONING):§7.7 line 293 改为 `PROVISIONING → ACTIVE` 并显式枚举全部项目状态,§14 增项目状态标签断言。P1(误报,已在 PR 评论写明反证)——codex 指本 docs 分支祖先不含 Plan D 实现(`StudioEndUserController`/`StudioJwtKeyController`/`EndUserAuthController` 缺失),依赖不满足:实为分支模型误解——§16.2 第 3 条明文「实现代码从 develop 新开 feature 分支,spec 修订在 docs 分支」,docs 分支按设计仅承载 spec、天然落后 develop;PR #16(Plan D)已 MERGED 入 develop(mergeCommit `828fe1b`,2026-07-23),develop 含上述控制器;PR #19 base=develop,合并后 docs 提交落在含 Plan D 的 develop 之上,Plan E 前端亦从 develop 开分支实现,依赖已满足,故不改动、不将 develop 合入 docs 分支(避免违背文档/实现分离决策)。本轮仅 P2 一处纯前端文案修正,不改后端契约与已合入代码。
