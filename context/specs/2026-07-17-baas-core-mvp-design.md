...
**接入层**:新增 `src/api/baas/{project,table,apiKey,endUser}.ts`,沿用 `utils/request.ts` 统一拦截(`R<T>` 解包、token 注入、401 跳转);**同步长操作(建/改/删表、ACL 配置、手动对账、建项目等走 §9.2 DDL 通道或同步执行的调用)须设一个独立可配、覆盖该操作端到端最坏耗时的 per-call client 超时,而非由后端单条 DDL 超时推导**——`baas.ddl.execute-timeout-seconds`(默认 300s,§13)是 per-statement 上限:建项目走 `ProjectProvisioner` 顺序执行多条 DDL(建库/建账号/授权/初始化系统表)且其 `JdbcTemplate` 未套用该超时,单条超时既非整请求上限也不覆盖 provisioner;client 超时取足够宽裕、独立于后端单条超时的可配值(表 DDL 类与建项目类可各自配置),明显大于最坏端到端耗时,不沿用 `utils/request.ts` 默认 30s Axios 超时;否则慢 ALTER/对账会在 UI 侧误报失败而服务端仍提交,慢建项目更会丢弃含两枚一次性初始 key 的响应;新增 env `VITE_BAAS_PATH` 默认 `/baas`(cloud 形态走网关剥首段),boot 单体部署覆盖为 `/admin`(同 `VITE_AUTH_PATH` 先例)。
