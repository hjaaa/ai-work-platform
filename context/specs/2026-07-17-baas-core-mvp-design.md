...
- 日期:2026-07-17
- 状态:v16;v1~v5 经四轮整体评审定稿,v6 附实施规划与会话交接(§16),v8 补齐 Plan B(表管理与 DDL)设计细化,v9~v16 按 Plan B 八轮设计评审修订
...
| 表 | 内容 |
|---|---|
| `baas_project` | `project_ref`(短标识,入 URL,**不作为授权凭据**)、`db_name`、**状态机字段**(见 9.1)、`owner_user_id`(平台用户,Studio 归属校验依据)、`allowed_origins`(JSON,CORS 白名单)、runtime 账号名及**加密**凭据、**`ddl_fence_epoch`(BIGINT NOT NULL,项目级单调 fencing 计数,见 9.2;Plan B 迁移新增)**。不存任何明文 key |
...
手动触发(`/reconcile`,body 携带客户端 `operationId`)+ 可选定时(`@Scheduled` + 配置开关,**默认关闭**,MVP 以手动为主)。对账日志记录**触发来源(MANUAL/SCHEDULED)**。**定时对账的陈旧任务发现(与 cleanup 调度器同型)**:定时器每轮**先扫描本项目已有的 SCHEDULED reconcile 记录**——FAILED → 执行 FAILED 重试 CAS;RUNNING 且 Redis 锁已不属其 owner_token → 陈旧 RUNNING 接管;两者皆无才以服务端生成的新 operationId 创建新操作(内部指纹见 §9.2);**同一项目同一时刻最多存在一个未终结(RUNNING 或 FAILED)的 SCHEDULED reconcile**——若每轮都直接换新 ID,遗留的 FAILED/陈旧 RUNNING 将永远无人认领。**范围仅表结构**:以项目库 `information_schema` 为准修正 `baas_table`/`baas_column`;ACL、owner 配置、密钥属于操作意图,以平台库为唯一事实源,不参与对账。**对账跳过非 ACTIVE 项目及软删/tombstone 表——物理表仍存在不构成「复活」软删除对象的依据**。**对账全程纳入 §9.2 统一日志与所有权模型**:操作类型 `reconcile`(项目级,目标表名与表 ID 为 NULL),走统一入口顺序与双层锁,按四分支取得 RUNNING 所有权;**对账产生的全部平台元数据修正、报告快照(`result_snapshot`)与 METADATA_APPLIED/SUCCESS 在同一平台库事务中提交,以 owner_token 条件更新作事务守卫**——读完 `information_schema` 后丢锁、他人完成并发 ALTER 的场景下,陈旧对账的整笔写入因守卫 0 行回滚,不会覆盖新元数据。对账报告即 `result_snapshot`(修正/导入/冲突清单),幂等重放返回原报告。
