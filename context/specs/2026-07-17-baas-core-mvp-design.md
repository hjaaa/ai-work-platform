...
**接入层**:新增 `src/api/baas/{project,table,apiKey,endUser}.ts`,沿用 `utils/request.ts` 统一拦截(`R<T>` 解包、token 注入、401 跳转);**同步长操作(建/改/删表、ACL 配置、手动对账、建项目等走 §9.2 DDL 通道或同步执行的调用)须按后端 5 分钟 DDL 超时(§13)设 per-call 更长 timeout(≥300s),不沿用 `utils/request.ts` 默认 30s Axios 超时**——否则慢 ALTER/对账会在 UI 侧误报失败而服务端仍提交,慢建项目更会丢弃含两枚一次性初始 key 的响应;新增 env `VITE_BAAS_PATH` 默认 `/baas`(cloud 形态走网关剥首段),boot 单体部署覆盖为 `/admin`(同 `VITE_AUTH_PATH` 先例)。
