...
- 日期:2026-07-17
- 状态:v19;v1~v5 经四轮整体评审定稿,v6 附实施规划与会话交接(§16),v8 补齐 Plan B(表管理与 DDL)设计细化,v9~v19 按 Plan B 十一轮设计评审修订
...
| 项目库有、元数据无(非 `_` 前缀) | 导入元数据,状态 ACTIVE,**ACL 默认全关**(安全兜底);**导入前先过下两行的主键与不支持结构检查,任一不过置 CONFLICT、不导入** |
| **主键不变量破坏**:缺少 `id` 列、`id` 非 bigint/非自增/非唯一主键、复合主键或存在额外主键(§13 固定主键约束) | 表置 CONFLICT(导入与修正两条路径均适用) |
| **不支持结构**:生成列(`information_schema.columns` 的 EXTRA/GENERATION_EXPRESSION 探测)、外键(约束表探测)等 §13 明确不支持的结构 | 表置 CONFLICT,不导入、不做普通修正(元数据模型无法表达,压缩导入会静默丢失结构信息) |
