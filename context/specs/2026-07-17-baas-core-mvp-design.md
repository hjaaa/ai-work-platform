...
- 项目列表页:members 页三件套范式(搜索卡 + 表格卡 + 创建弹窗);状态列覆盖 §9.1 全部项目状态(`PROVISIONING/ACTIVE/FAILED/DELETING/DELETED`,同 `ProjectStatus` 枚举),浅底深字标签;`PROVISIONING → ACTIVE` 由用户手动刷新,MVP 不做轮询
