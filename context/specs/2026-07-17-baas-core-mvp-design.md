...
- 日期:2026-07-17
- 状态:v24;v1~v5 经四轮整体评审定稿,v6 附实施规划与会话交接(§16),v8 补齐 Plan B(表管理与 DDL)设计细化,v9~v24 按 Plan B 十六轮设计评审修订
...
| 表 | 内容 |
|---|---|
| `_users` | 终端用户:id(**固定 signed bigint 自增**,owner 列类型与其对齐,全链路以 Java Long 承载;**Plan A 现实现为 bigint unsigned,Plan B 迁移统一改为 signed**,含 `_sessions.user_id`、`_refresh_tokens.session_id/replacement_token_id` 等关联列与建表模板)、email(trim + 小写规范化后唯一)、password_hash(bcrypt)、raw_meta(JSON)、时间戳 |
...
- 系统表 DDL **显式携带完整物理基线**(ENGINE/CHARSET/COLLATE/ROW_FORMAT;Plan A 现有语句仅写 ENGINE,Plan B 须补);因 `CREATE TABLE IF NOT EXISTS` 可能命中预存表,初始化后**回读三张系统表及其字符串列的实际基线**,不符则执行修正迁移或将项目置 FAILED——**不得置 ACTIVE**(对账跳过 `_` 系统表,错误基线此后无人拦截)
...
| **B 表管理与 DDL** | 建/改/删表(元数据 + 真实 DDL,改表为全能档:加/删列、改类型/长度、重命名列/表,含 `allowLossy` 确认与类型兼容矩阵)、Redis 串行锁 + 操作 ID 幂等(`baas_ddl_log`)、表状态机(§9.5)、表级 ACL 与 owner 列配置(bigint 校验/自动建索引/anon.insert 要求可空)、软删 tombstone 与同名禁重建、`information_schema` 对账 | §7.3(表管理契约)、§8.2/8.3(配置面)、§9.2/9.4/9.5、§13(表编辑器边界与兼容矩阵) | A | 设计细化至 v24(v8 后经十六轮评审修订,v24 消除十六轮 3 P1;范围含项目删除/清理取锁改造、项目级 epoch fencing、ACTIVE 准入谓词、类型规范化与参数矩阵、索引名分配器与索引准入矩阵、物理基线 fail-closed、系统表 signed 迁移),待复审;计划未写 |
