...
- 日期:2026-07-17
- 状态:v23;v1~v5 经四轮整体评审定稿,v6 附实施规划与会话交接(§16),v8 补齐 Plan B(表管理与 DDL)设计细化,v9~v23 按 Plan B 十五轮设计评审修订
...
- **索引准入矩阵**(create/add/modify/索引开关在进入日志所有权事务前按**修改后的最终结构**统一校验,任一不过 400、不执行 DDL):① `text`/`json` 列 `unique=true` 或 `indexed=true` 一律 400(text 的 BTREE 索引必须指定前缀而前缀索引已禁止;json 需生成列或函数索引而两者均禁止);② varchar 索引键长按 utf8mb4 计算 **`length × 4 ≤ 3072` 字节**(前置条件 `innodb_page_size=16KiB` + ROW_FORMAT=DYNAMIC,即全列索引 length ≤ 768;**modify 扩长已带索引的 varchar 越界同样 400**);③ 最终**二级索引总数 ≤ 64**(InnoDB 上限)
...
4. **已确认的工程事实**(写计划时直接采用):
   ...
   - Plan A 的 `ProjectProvisioner.CREATE_DATABASE_SQL` 已显式 `DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci`,与 §13 物理基线一致;部署前置条件 `innodb_page_size=16KiB`(§13 varchar 键长校验按此计算)
