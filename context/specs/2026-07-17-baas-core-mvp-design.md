...
- 项目列表页:members 页三件套范式(搜索卡 + 表格卡 + 创建弹窗);状态列覆盖**列表 API 实际可返回的项目状态**(`PROVISIONING/ACTIVE/FAILED/DELETING`——`ProjectAccessService.listVisible` 与单查均 `ne DELETED`,DELETED 项目对 Studio 不可见,故不在状态列之列),浅底深字标签;`PROVISIONING → ACTIVE` 由用户手动刷新,MVP 不做轮询
...
**接入层**:新增 `src/api/baas/{project,table,apiKey,endUser}.ts`,沿用 `utils/request.ts` 统一拦截(`R<T>` 解包、token 注入、401 跳转);**同步长操作(建/改/删表、ACL 配置、手动对账、建项目等走 §9.2 DDL 通道或同步执行的调用)须设 per-call timeout 严格大于后端可配 DDL 超时(`baas.ddl.execute-timeout-seconds`,默认 300s,§13)并留明确余量**——后端命中 DDL 超时后还需转译失败、更新 DDL 日志与表状态、提交事务再返回响应,客户端 timeout 恰好等于该值会先 abort;取「后端配置超时 + 缓冲(如 30~60s)」,不沿用 `utils/request.ts` 默认 30s Axios 超时;否则慢 ALTER/对账会在 UI 侧误报失败而服务端仍提交,慢建项目更会丢弃含两枚一次性初始 key 的响应;新增 env `VITE_BAAS_PATH` 默认 `/baas`(cloud 形态走网关剥首段),boot 单体部署覆盖为 `/admin`(同 `VITE_AUTH_PATH` 先例)。
...
- **Plan E(Studio 前端)必测**(Vitest 单测,前端门禁):**改表操作列表组装**(重命名与删+加的区分、同列多操作互斥在前端拦截、无操作禁提交、组装出的 PATCH body 与 §7.3 契约字段逐项一致);**列定义校验边界**(varchar 长度 1/4096/越界、decimal 精度/标度约束、text/json 默认值拦截、datetime `CURRENT_TIMESTAMP` 特例);**allowLossy 重发流程**(识别「需要 allowLossy」的 400 → 确认后同 operationId 重发,body 仅 `allowLossy` 变化);**`VITE_BAAS_PATH` 前缀拼接**(默认 `/baas` 与 boot 覆盖 `/admin`);detail 静态子路由注册与守卫回归(并入现有 guard/menuPaths spec 覆盖);**侧边栏前缀高亮**(`/baas/projects/:ref` 详情路由下父菜单 `/baas/projects` 高亮、`/baas/projects-x` 类同前缀平级路径不误亮);**`allowed_origins` 通配/白名单往返**(`ProjectVO` 回 `null` → 通配开关开启、回 `[]` → 开关关闭且空列表、编辑提交值与开关态一致,不把通配存成 deny-all);**建表 body 剔除 id 行**(只读 id 行不进 `columns`,POST body 断言无 `id` 列定义);**409 分类提示**(锁忙 409 → 「刷新重试」文案、指纹/有损/唯一键等非锁 409 → 直显后端 msg,不误导重试);**创建响应双 key 展示**(建项目弹窗同时展示 publishable 与 secret 明文并可复制);**项目状态标签**(列表 API 可见的 `PROVISIONING/ACTIVE/FAILED/DELETING` 各有标签样式——DELETED 被 `listVisible` 过滤不可达、不纳入,PROVISIONING 文案与手动刷新提示正确);**operationId 明文 HTTP 回退**(`crypto.randomUUID` 缺失时以 `crypto.getRandomValues` 生成合法 UUID v4,重试复用同一 ID);**长操作 timeout 覆盖**(DDL/对账/建项目类调用的 per-call timeout 严格大于后端可配 DDL 超时 + 余量,不落 30s 默认);**owner 列下拉排除 id**(仅 bigint 且不含主键 id,避免选中必被后端 400 拒的 id)。`npm run lint` / `npm run build`(vue-tsc)/ `npm run test:unit -- run` 三门禁全绿;依赖变更须干净环境 `npm ci` 验证
