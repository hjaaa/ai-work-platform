...
3. 【强制】服务端发生错误时，返回给前端的响应必须包含 HTTP 状态码与统一响应体 `R{code, msg, data}`。
    ...
    **【项目调整】** 原书要求的"errorCode、errorMessage、用户提示信息"在本项目收敛为 `code` 与 `msg` 两个字段：`code` 是成功/失败标记（int 型，取值见 `CommonConstants`，不采用原书附表 3 的五位错误码体系）；`msg` 是**用户可见文案**（前端拦截器直接以消息弹窗展示），要求简短清晰、提示友好，引导用户进行下一步操作或解释错误原因。排障细节（堆栈、入参、上下文环境）只记录在服务端日志，禁止塞进 `msg`，注意不要包含敏感数据信息。
