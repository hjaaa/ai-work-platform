...
/**
 * PRD 生成服务
 * 通过多轮对话将自然语言转化为结构化 PRD 文档
 */
@Service
public class PrdGenerateService {

    private static final Logger log = LoggerFactory.getLogger(PrdGenerateService.class);
    ...
    public PrdGenerateService(ClaudeCodeOrchestrator orchestrator,
        ...
        try {
            ...
            if (parsed.prdContent() != null) {
                messagePushService.pushToProject(projectId,
                        com.aiworkplatform.service.chat.ChatMessage.builder()
                                .projectId(projectId)
                                .role("assistant")
    ...
    /**
     * 解析 Claude Code 输出，提取 PRD 内容和追问问题
     */
